...
async fn setup_game_stream(game_id: String) -> Result<()> {
    let mut game_stream = CLIENT
        .get(format!(
    ...
    while let Some(event) = game_stream.next().await {
        let event_bytes = &event.unwrap();
        ...
        println!("{}", event_str);
        //if gameFull set game
        //if gameState set delta

        let res = CLIENT
            .post(format!(
                "https://lichess.org/api/bot/game/{}/move/{}?offeringDraw=true",
                game_id, "e7e6"
            ))
            .send()
            .await?;
        ...
        match event.event_type {
            ...
            EventType::GameEvent(game) => {
                if event.type_string == "gameStart" {
                    ...
                    //run game stream
                    setup_game_stream(game.game_id).await?;
                } else {
                    println!(
                        "Received {} from {}; id: {}",
